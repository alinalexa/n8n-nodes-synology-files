...
export class SynologyFileStation implements INodeType {
	description: INodeTypeDescription = {
		...
		properties: [
			...
			},
			{
				...
				description:
					'For List: folder path. For Download/Delete/Rename: full path to the file/folder.',
			},
			{
				displayName: 'Offset',
				name: 'offset',
			...
			},
			{
				...
				description:
					'Binary property containing the file to upload or where to store the downloaded file',
			},
			{
			...
			},
			{
				...
				description:
					'JSON array of files/folders to move. Example: ["\\/folder\\/a.txt","\\/folder\\/b.txt"].',
			},
			{
	...
	async execute(this: IExecuteFunctions) {
		...
		const sid = await synoLogin.call(this);
		...
		for (let i = 0; i < items.length; i++) {
			if (operation === 'list') {
				...
				const res = await synoRequestJson.call(this, sid, {
					method: 'GET',
				...
				results.push({ json: (res.data as IDataObject) ?? res });
			}
			...
			if (operation === 'upload') {
				...
				const res = await synoPostMultipart.call(this, sid, {
					url: '/webapi/entry.cgi',
					qs: {
			...
			if (operation === 'download') {
				...
				const buf = await synoRequestBinary.call(this, sid, {
					method: 'GET',
					url: '/webapi/entry.cgi',
					qs: {
			...
			if (operation === 'delete') {
				const path = this.getNodeParameter('path', i) as string;
				const recursiveDelete = this.getNodeParameter('recursiveDelete', i) as boolean;
			...
			if (operation === 'move') {
				const sourcePathsRaw = this.getNodeParameter('sourcePaths', i) as string;
				const targetPath = this.getNodeParameter('targetPath', i) as string;
				...
				try {
					...
					if (!Array.isArray(sourcePaths))
						throw new NodeOperationError(
							this.getNode(),
							'“Source Paths (JSON Array)” must be a JSON array of strings.',
						);
				} catch {
					throw new NodeOperationError(
						this.getNode(),
